...
info_get = function(x, info){
  if(str_split(x, "/")[[1]][3] == "2 subject sample"){
    b = str_split(x, "/")[[1]][length(str_split(x, "/")[[1]])]
    if(info == "visit"){
...
mix_model = function(outcome, thre = NULL, df, type = "raw", lambda = NULL, features = NULL){
  if(type == "raw"){
    ...
    result_table = model %>% tidy() %>% filter(term %in% c("mean_delta_pct", "sd_delta_pct", "sd_PCO2", "delta_pco2_time", "delta_pco2_event", PC_time, event)) %>% mutate(result = outcome, PCO2_thre = thre) %>% dplyr::select(term, estimate, p.value, result, PCO2_thre)
    result_table$estimate = sprintf("%.3f",result_table$estimate)}else if(type == "pca") {
    ...
    if (grepl("alsfrs", outcome)){
      ...
      model = glmmLasso(as.formula(formula),
          ...
          switch.NR = TRUE,
          final.re = TRUE)}
      result_table = data.frame(summary(model)$coefficients)[-1,] %>% filter(p.value < 0.5) %>% dplyr::select("Estimate", "p.value") %>% rownames_to_column() %>% mutate(
        p.value = case_when(p.value > 0.1 ~ "*",
                            p.value > 0.01 & p.value <= 0.1~ "**",
                            p.value < 0.1~ "***"), result = outcome)
      colnames(result_table) = c("term", "estimate", "sig", "result") 
      result_table = result_table[c("result", "term", "estimate", "sig")]
      result_table$estimate = sprintf("%.3f",result_table$estimate)
    }
...
majority_vote = function(k_means_df){
  mat = as.matrix(k_means_df[features]) 
  hclust.out = hclust(dist(mat))
  k_means_df$cluster = cutree(hclust.out, k = 3)
  k_means_df$cluster = as.factor(k_means_df$cluster)
...
nodes_f = function(s, k_means_df){
  df = k_means_df %>% arrange(subject, day) %>% dplyr::select(subject, visit, day, cluster_night, cluster_visit, cluster) %>% distinct() %>% filter(subject == s) 
  ...
  nodes_subject = df %>% mutate(variable = subject) %>% dplyr::select(variable, cluster) %>% rename("group" = "cluster") %>% distinct()
...
vizNetwork = function(vis.nodes, vis.edges){
  colrs = setNames(c("gray", "gray", "gray", "salmon", "darkorchid", "cyan"), 
                   c("Group A/B", "Group A/C", "Group B/C", "Group A", "Group B", "Group C"))
  # adapt to variable 
  vis.nodes$shape  <- "dot"  
  ...
  vis.nodes$color.border <- "black"
    vis.nodes$color.highlight.background <- "orange"
      ...
      visNetwork(vis.nodes, vis.edges, width = "100%") %>% 
        ...
        visHierarchicalLayout() %>% visGroups(groupname = "Group A/B", color = 'gray', shape = "dot", color.border = 'black') %>% visGroups(groupname = "Group A", color = 'salmon', shape = "dot", color.border = 'black') %>% visGroups(groupname = "Group B", color = 'darkorchid', shape = "dot", color.border = 'black') %>% visGroups(groupname = "Group C", color = 'cyan', shape = "dot", color.border = 'black')%>% visGroups(groupname = "Group A/C", color = 'gray', shape = "dot", color.border = 'black')%>% visGroups(groupname = "Group B/C", color = 'gray', shape = "dot", color.border = 'black') %>% visNodes(font = list(color = "white")) 
}

cluster_test = function(outcome, df){
  form = paste0(outcome, " ~ cluster_visit + visit_time + (1 | subject)")
  model = lmerTest::lmer(as.formula(form), data = df)
  result_table = model %>% tidy() %>% filter(grepl("visit", term)) %>% mutate(term = gsub("cluster_visit", "", term), sig = case_when(p.value< 0.1 & p.value >= 0.05 ~ ".",
                                                                                                                                      p.value < 0.05 & p.value >= 0.01 ~ "*",
                                                                                                                                      p.value < 0.01 & p.value >= 0.001 ~ "**",
                                                                                                                                      p.value < 0.001 ~ "***",
                                                                                                                                      .default = NA)) %>% dplyr::select(term, estimate, sig) %>% mutate(outcome = outcome, .before = "term", estimate = round(estimate, 3))
  return(result_table)
}
