...
sample_info = function(subject_summary){
  ...
  N_visits = subject_summary %>% group_by(subject, visit) %>% summarize(count = n()) %>% ungroup() %>% nrow()
  n_dc_4 = nrow(subject_summary %>% filter(DC == "DC", period == ">= 4 hours"))
  n_dc_4_visit = subject_summary %>% filter(DC == "DC", period == ">= 4 hours") %>% group_by(subject, visit) %>% summarize(count = n()) %>% ungroup() %>% nrow() 
  ...
  name = c("Total Records", "Records (with DC & t >= 4h)", "Total Visits", "Total Visits (with DC & t >= 4h)", "Records Used", "Records (CO2 > 45 for > 5 mins)", "Records (CO2 > 45 for > 10 mins)",
  ...
  count = c(N, n_dc_4, N_visits, n_dc_4_visit, n_used, event_45_5, event_45_10, event_50_10, delta_10_50, fvc_above_50, mip_above_60, SpO2_less_5min, norm_com, 
